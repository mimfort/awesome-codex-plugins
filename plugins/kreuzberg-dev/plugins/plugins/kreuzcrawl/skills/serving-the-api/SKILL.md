...
description: >-
  ...
  kreuzcrawl into other apps over REST. Covers `kreuzcrawl serve`, the
  Firecrawl-v1-compatible endpoints, `--host`/`--port`, and when to prefer it.
---

# Serving the API

`kreuzcrawl serve` starts a long-running REST API server exposing the crawl
engine over HTTP. The endpoints are Firecrawl v1-compatible, so existing
Firecrawl clients and SDKs can point at a local kreuzcrawl instance.
...
- **MCP** (`kreuzcrawl mcp`) — driving the crawler from an AI agent harness
  (Claude Code, Codex, Cursor, Gemini, opencode). Auto-registered by this
  plugin; prefer it inside an agent.
- **serve** (`kreuzcrawl serve`) — a persistent HTTP service for other
